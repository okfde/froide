...
class FoiRequestFollowerTest(TestCase):
    ...
    def test_updates_avoid(self):
        ...
        req2 = factories.FoiRequestFactory.create(site=self.site, user=req.user)
        mes = list(req.messages)[-1]
...
class ApiTest(OAuthAPIMixin, TestCase):
    ...
    def test_following_requests_unauth(self):
        ...
        self.assertTrue(
            result["objects"][0]["request"].endswith(
                "/request/{}/".format(self.other.id)
            )
        )
    ...
    def test_follow_unauth(self):
        request_ids = (self.accessible.id, self.inaccessible.id, self.other.id)
    ...
    def test_follow_loggedin(self):
        ...
        response = self.client.post(
            self.follow_url,
            json.dumps({"request": self.accessible.id}),
            content_type="application/json",
        )
        ...
        response = self.client.post(
            self.follow_url,
            json.dumps({"request": self.inaccessible.id}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 400)
        ...
        response = self.client.post(
            self.follow_url,
            json.dumps({"request": self.other.id}),
            content_type="application/json",
        )
    ...
    def test_follow_oauth_no_scope(self):
        ...
        request_ids = (self.accessible.id, self.inaccessible.id, self.other.id)
        for request_id in request_ids:
            response, result = self.api_post(self.follow_url, {"request": request_id})
    ...
    def test_follow_auth_with_scope(self):
        ...
        response, result = self.api_post(
            self.follow_url, {"request": self.accessible.id}
        )
        self.assertEqual(response.status_code, 400)

        response, result = self.api_post(
            self.follow_url, {"request": self.inaccessible.id}
        )
        self.assertEqual(response.status_code, 400)

        response, result = self.api_post(self.follow_url, {"request": self.other.id})
        self.assertEqual(response.status_code, 201)
        ...
        resource_uri = result["objects"][0]["resource_uri"]
        resource_uri_relative = "/".join([""] + resource_uri.split("/")[3:])
        response, result = self.api_delete(resource_uri_relative)
        self.assertEqual(response.status_code, 204)
        response, result = self.api_get(self.follow_url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(result["meta"]["total_count"], 0)
    ...
    def test_delete_other(self):
        ...
        delete_url = reverse(
            "api:following-detail", kwargs={"pk": ff_obj.id}
        ) + "?request={}".format(ff_obj.id)
        response = self.client.delete(delete_url, content_type="application/json")
        self.assertEqual(response.status_code, 404)

        delete_url = reverse("api:following-detail", kwargs={"pk": ff_obj.id})
        response = self.client.delete(delete_url, content_type="application/json")
        self.assertEqual(response.status_code, 404)
