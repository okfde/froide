...
class WebTest(TestCaseHelpers, TestCase):
    ...
    def test_request_prefilled_redirect(self):
        ...
        response = self.client.get(
            reverse("foirequest-make_request", kwargs={"publicbody_ids": str(p.pk)})
            + query
        )
    ...
    def test_publicbody_requests(self):
        ...
        pb = req.public_body
        response = self.client.get(
            reverse("foirequest-list", kwargs={"publicbody": pb.slug})
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, req.title)
        response = self.client.get(
    ...
    def test_short_link_request(self):
        req = FoiRequest.objects.all()[0]
        response = self.client.get(
            reverse("foirequest-shortlink", kwargs={"obj_id": 0})
        )
        self.assertEqual(response.status_code, 404)
        response = self.client.get(
    ...
    def test_feed(self):
        ...
        self.assertRedirects(response, reverse("foirequest-list_feed"), status_code=301)
        response = self.client.get(reverse("foirequest-feed_latest_atom"))
        self.assertRedirects(
            response, reverse("foirequest-list_feed_atom"), status_code=301
        )
        ...
        req = FoiRequest.objects.all()[0]
        response = self.client.get(
            reverse("foirequest-feed_atom", kwargs={"slug": req.slug})
        )
        self.assertEqual(response.status_code, 200)
        response = self.client.get(
            reverse("foirequest-feed", kwargs={"slug": req.slug})
        )
        self.assertEqual(response.status_code, 200)
...
class MediaServingTest(TestCaseHelpers, TestCase):
    ...
    def test_request_not_public(self):
        ...
        response = self.client.get(response["Location"], HTTP_HOST=MEDIA_DOMAIN)
        self.assertEqual(response.status_code, 200)
        self.assertIn("X-Accel-Redirect", response)
        self.assertEqual(
            response["X-Accel-Redirect"],
            "%s%s" % (settings.INTERNAL_MEDIA_PREFIX, att.file.name),
        )
    ...
    def test_request_media_tokens(self):
        ...
        self.assertForbidden(response)
        loggedin = self.client.login(email="info@fragdenstaat.de", password="froide")
        ...
        self.assertEqual(
            response["Link"],
            '<{}>; rel="canonical"'.format(att.get_absolute_domain_url()),
        )
