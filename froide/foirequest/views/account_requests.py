...
class MyBaseListRequestView(BaseListRequestView):
    ...
    advanced_filters = {"first", "project", "sort"}
    model = FoiRequest
...
class MyRequestsView(BaseAccountMixin, MyBaseListRequestView):
    ...
    def get_base_search(self):
        return self.document.search().filter("term", user=self.request.user.pk)
...
class DraftRequestsView(BaseAccountMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return RequestDraft.objects.filter(user=self.request.user, **query_kwargs)
...
class FoiProjectListView(BaseAccountMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if self.query:
            query_kwargs = {"title__icontains": self.query}
        return FoiProject.objects.get_for_user(self.request.user, **query_kwargs)
...
class UserRequestFeedView(ListRequestView):
    ...
    def get_queryset(self):
        ...
        self.filtered_objs = {"user": user}
        self.filter_data = {"user": token}
...
def user_calendar(request, token):
    ...
    queryset = (
        FoiRequest.objects.filter(user=user)
        .filter(
            Q(due_date__gte=three_months_ago) | Q(last_message__gte=three_months_ago)
        )
        .select_related("law")
    )
    ...
    cal.add(
        "prodid",
        "-//{site_name} //{domain}//".format(
            site_name=settings.SITE_NAME, domain=settings.SITE_URL.split("/")[-1]
        ),
    )
    cal.add("version", "2.0")
