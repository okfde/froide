...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for req in orm['foirequest.FoiRequest']._default_manager.all():
            if req.status in RESOLUTION_CHOICES:
                req.resolution = req.status
                req.status = 'resolved'
                req.save()
            elif req.status not in STATUS_CHOICES:
