...
def export_user_data(user):
    ...
    user_data = get_dict(
        ...
        (
            ...
            "is_staff",
            "profile_text",
            ("profile_photo", lambda x: os.path.basename(x.path) if x else None),
    ...
    if apps:
        ...
        yield (
            "apps.json",
            json.dumps([get_dict(a, fields) for a in apps]).encode("utf-8"),
        )
    grants = user.oauth2_provider_grant.all().select_related("application")
    accesstokens = user.oauth2_provider_accesstoken.all().select_related("application")
...
class ExportCrossDomainMediaAuth(CrossDomainMediaAuth):
    ...
    def get_auth_url(self):
        ...
        with override(settings.LANGUAGE_CODE):
            return reverse("account-download_export_token", kwargs={"token": obj.token})
