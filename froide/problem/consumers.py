...
async def is_moderator(scope):
    ...
    return await database_sync_to_async(is_foirequest_moderator)(ScopeRequest(scope))
...
class ModerationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_add(PRESENCE_ROOM, self.channel_name)
        await self.accept()
    ...
    async def send_userlist(self, action="joined"):
        ...
        moderators = [
            {"id": u.id, "name": None if u.private else u.get_full_name()}
            for u in users
        ]
