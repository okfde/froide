<template>
  <div>
      ...
      >
        <div class="form-check">
          ...
          <label class="form-check-label text-break" :for="item.labelId">
            {{ item.label }}
            <small v-if="item.subLabel" class="text-secondary d-block">{{
              item.subLabel
            }}</small>
          </label>
          <small v-if="item.count" class="filter-list-sublabel"
            >({{ item.count }})</small
          >
          <i
    ...
    <ul v-else class="list-unstyled mb-0 filter-list--grouped">
      <li v-for="itemGroup in itemGroups" :key="itemGroup.id">
        <!-- special simple case for groups with one member -->
        <div v-if="itemGroup.items.length === 1" class="form-check">
          <input
            :type="inputType"
            :name="inputName"
            class="form-check-input"
          ...
          <label
            ...
            :for="itemGroup.items[0].labelId"
          >
            {{ itemGroup.items[0].label }}
          </label>
          <small v-if="itemGroup.items[0].count"
            >({{ itemGroup.items[0].count }})</small
          >
        </div>
        ...
        <div v-else>
          <div class="form-check">
            ...
            />
            <label class="form-check-label text-break" :for="itemGroup.id">{{
              itemGroup.label
            }}</label>
            <i
