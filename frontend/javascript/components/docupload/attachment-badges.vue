<template>
  <AttachmentBadgeFiletype v-if="badgesType" :attachment="attachment" />
  <span
  ...
  <span v-if="badgesNew && attachment.new" class="badge text-bg-success me-1">{{
    i18n.new
  }}</span>
  <span
...
const { attachment, badgesNew, badgesRedaction, badgesType, badgesResolution } =
  defineProps({
    attachment: {
      type: Object,
      required: true
