...
const pages = computed(() => {
  ...
  for (let p = 0; p < total; p++) {
    ...
    if (
      p < nearFirstLast ||
      p >= total - nearFirstLast ||
      Math.abs(p - current) <= nearCurrent
    ) {
      collapsing = false
      ret.push({
        key: p,
...
<template>
  <nav :aria-label="i18n.pagination" v-show="show">
    <ul class="pagination flex-wrap">
      ...
      >
        <span class="page-link" @click="model.offset = page.offset">
          <span v-if="page.ariaHiddenLabel" aria-hidden="true">{{
            page.ariaHiddenLabel
          }}</span>
          <span v-if="page.visuallyHiddenLabel" class="visually-hidden">{{
            page.visuallyHiddenLabel
          }}</span>
